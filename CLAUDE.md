...
- `.github/workflows/publish.yml`: manual-only (`workflow_dispatch` with a `release` choice of `patch`/`minor`/`major`). It re-runs typecheck/test/build, then runs `pnpm bumpp --yes --release <type>` (bumps version, commits, tags, and pushes to `main`), then `pnpm publish --provenance`.
- Requires an `NPM_TOKEN` repo secret (npm "Automation" access token) — publish will fail without it. Nothing publishes automatically; a maintainer must trigger the workflow from the Actions tab.
